...
-- Create updated_at trigger for organizations (with IF NOT EXISTS)
DO $$
BEGIN
    IF NOT EXISTS (
...
-- Create updated_at trigger for organization_members (with IF NOT EXISTS)
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_trigger 
...
-- Fix the recursive policy by using direct comparison instead of EXISTS subquery
...
-- Fix the policy for inserting members to allow the first member (creator) to be added
DROP POLICY IF EXISTS "Admins can add members to their organizations" ON public.organization_members;
CREATE POLICY "Users can add members to their organizations" ON public.organization_members
    FOR INSERT
    WITH CHECK (
...
DROP POLICY IF EXISTS "Admins can update members in their organizations" ON public.organization_members;
CREATE POLICY "Admins can update members in their organizations" ON public.organization_members
    FOR UPDATE
    USING (public.is_organization_admin(organization_id));

DROP POLICY IF EXISTS "Admins can remove members from their organizations" ON public.organization_members;
